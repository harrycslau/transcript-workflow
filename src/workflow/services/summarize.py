...
def _final_system_prompt(tags: list[Tag]) -> str:
    return (
        ...
        "Rules:\n"
        "- Write `title` and `overview` in the dominant language used by the speaker. "
        "Mixed Cantonese/English content may produce Cantonese prose while preserving "
        "necessary English terminology.\n"
        "- `key_points` are short important statements from the transcript.\n"
        "- `action_items`: only actions stated or directly implied by the transcript. "
        "Use null for `owner` and `due_date` when they are not stated. Never fabricate. "
        "An empty list is valid.\n"
        "- `people`, `organizations`, `topics`: only names and topics that appear in "
        "the transcript.\n"
...
def _map_system_prompt() -> str:
    return (
        ...
        "Rules:\n"
        "- Write in the dominant language used in the chunk. Mixed Cantonese/English "
        "content may produce Cantonese prose while preserving necessary English "
        "terminology.\n"
        "- `overview` is a concise, self-contained summary of THIS chunk; `key_points` "
        "are short important statements.\n"
        "- Never fabricate information not present in the chunk.\n\n"
        f"Respond with ONLY a JSON object with exactly this shape:\n{MAP_SHAPE_DOC}"
    )
...
def _reduce_user_prompt(intermediates: list[dict], *, final: bool) -> str:
    ...
    if final:
        instruction = (
            ...
            "recording and respond with ONLY a JSON object with exactly this shape:\n"
            f"{FINAL_SHAPE_DOC}"
        )
    else:
        instruction = (
            "These are partial summaries of consecutive chunks of one transcript, in "
            "chronological order. Merge them into one shorter partial summary and "
