...
class TestReviewCategories:
    def test_all_categories_present_with_items(self, client):
        ...
        # The obsolete "Transcribed with unverified automatic routing"
        # category is gone.
        assert "unverified automatic routing" not in content
        for recording_pk in (
            needs_review.pk, retx_failed.pk, failed.pk,
    ...
    def test_empty_states(self, client):
        ...
        assert response.status_code == 200
        content = response.content.decode()
        assert content.count("Nothing here. Good.") >= 7
    ...
    def test_group_slugs_are_stable(self, client):
        ...
        assert slugs == [
            "needs_review", "retranscription_failed", "failed",
...
class TestReviewQueryScaling:
    ...
    def test_report_data_correct_across_all_categories(self, client):
        ...
        # The unverified-automatic-routing category is audit-only and
        # never populated (the empty key is retained for shape only).
    ...
    def test_no_per_row_decision_or_attempt_queries(self, client):
        ...
        # One filtered prefetch for needs_review + one for the narrowed
        # failed-retranscription set — never one per row.
        ...
        # comes from select_related on the failed-retranscription query.
        assert len(attempt_queries) <= 2, attempt_queries

    def test_unverified_only_transcribed_not_loaded_for_review(self):
        """The failed-retranscription query is narrowed to
        ``retranscription_failed=True``, so an unverified-only transcribed
        Recording is neither loaded nor decision-prefetched for review."""
        from workflow.services.review import build_review_report

        unverified, _t, _s = make_transcribed_recording(["x"], sha="rev-unv-load")
        make_summary_version(unverified, _t, _s)
        _decision(unverified, verified=False)
        failed_retx, _t2, _s2 = make_transcribed_recording(["x"], sha="rev-retx-load")
        attempt = ProcessingAttempt.objects.create(
            recording=failed_retx, stage=AttemptStage.TRANSCRIPTION, ordinal=2,
            outcome=AttemptOutcome.NONZERO_EXIT, error_code="mw_exit_1", finished_at=None,
        )
        Recording.objects.filter(pk=failed_retx.pk).update(
            retranscription_failed=True, last_failed_attempt=attempt
        )
        ...
        assert unverified.pk not in [
            item["recording_id"] for items in report.values() for item in items
        ]
        assert failed_retx.pk in [
            item["recording_id"] for item in report["failed_retranscription"]
        ]

        # The select_related failed-retranscription query (the only
        # status-scoped Recording query joined to processingattempt) is
        # filtered on retranscription_failed at the DB level; the broad
        # transcribed scan is gone.
        joined_status_queries = [
            q["sql"] for q in ctx.captured_queries
            if "workflow_recording" in q["sql"]
            and "workflow_processingattempt" in q["sql"]
            and "processing_status" in q["sql"]
        ]
        assert joined_status_queries, "failed-retranscription query not found"
        for sql in joined_status_queries:
            assert "retranscription_failed" in sql
    ...
    def test_cli_review_json_schema_unchanged(self, capsys):
        ...
        # Shape-compat only: the key exists but is never populated.
...
class TestUnverifiedAutomaticRoutingIsAuditOnly:
    """An active UNVERIFIED automatic RoutingDecision on a transcribed
    Recording is audit-only on every actionable-review surface, while a
    true pre-transcription NEEDS_REVIEW still reports. Covers both an
    existing (pre-change) row shape and a freshly created one — the
    behavior derives only from persisted state, never a migration."""

    def _audit_only_recording(self, sha):
        recording, transcript, section = make_transcribed_recording(["x"], sha=sha)
        make_summary_version(recording, transcript, section)
        _decision(recording, verified=False)
        return recording

    def test_report_never_reports_unverified_as_attention(self):
        from workflow.services.review import build_review_report

        recording = self._audit_only_recording("audit-report")
        report = build_review_report()
        assert report["unverified"] == []
        for key, items in report.items():
            assert recording.pk not in [item["recording_id"] for item in items], key
        # The decision row itself is untouched.
        decision = RoutingDecision.objects.get(recording=recording, is_active=True)
        assert decision.routing_verified is False

    def test_review_page_omits_category_and_recording(self, client):
        recording = self._audit_only_recording("audit-page")
        response = client.get("/review/")
        content = response.content.decode()
        assert "unverified automatic routing" not in content
    ...
    def test_decision_remains_unverified_and_visible(self, client):
        recording = self._audit_only_recording("audit-visible")
        decision = RoutingDecision.objects.get(recording=recording, is_active=True)
        detail = client.get(f"/recordings/{recording.pk}/").content.decode()
        history = client.get(f"/recordings/{recording.pk}/history/").content.decode()
        decision.refresh_from_db()
        assert decision.routing_verified is False
        # Routing metadata/detail disclosure keeps advertising the
        # unverified state; history still shows the decision.
        assert "unverified" in detail
        assert "cantonese" in history

    def test_true_needs_review_still_reported(self, client):
        from workflow.services.review import build_review_report

        recording, _t, _s = make_transcribed_recording(["x"], sha="audit-needs-review")
        Recording.objects.filter(pk=recording.pk).update(
            processing_status=ProcessingStatus.NEEDS_REVIEW
        )
        _decision(recording)
        ...
        report = build_review_report()
        assert recording.pk in [i["recording_id"] for i in report["needs_review"]]
        response = client.get("/review/")
        groups = {g["key"]: g for g in response.context["groups"]}
